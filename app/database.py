...
Слой совместимости с интерфейсом asyncpg-пула (`db.acquire()`, `fetch`,
`fetchrow`, `fetchval`, `execute`, плейсхолдеры $1..$N), чтобы торговая
логика не зависела от конкретной СУБД. Для одиночного бота в контейнере
SQLite достаточно: нет внешних зависимостей и отдельного сервера БД.
"""
import logging
import os
import re
