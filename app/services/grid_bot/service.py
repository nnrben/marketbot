...
from app.services.grid_bot.exceptions import (
    BotOperationInProgress,
    ExchangeClosedError,
    _is_exchange_closed_error,
)
...
# Боты, ожидающие открытия биржи. Ключ — bot_id. Значение — метаданные,
# нужные фоновому планировщику (_waiting_scheduler_loop), чтобы понять, по
# какой бирже ждать открытия. Пока бот в этом словаре, его статус в БД —
# 'waiting'. Как только биржа открывается, планировщик сам вызывает
# _do_start_bot и убирает бот из очереди.
_waiting_bots: Dict[int, dict] = {}
_waiting_lock = asyncio.Lock()
_waiting_scheduler_task: Optional[asyncio.Task] = None


# Боты, по которым прямо сейчас выполняется мутирующая операция,
# инициированная по HTTP (запуск/остановка/удаление). Пока bot_id здесь,
# повторный такой же запрос по этому боту отклоняется (см. bot_operation_guard).
# Это защищает от дубль-команд с фронта: раньше повторное «удалить и продать»,
# отправленное до завершения долгой рыночной продажи, запускало вторую
# ликвидацию и рапортовало «продано 0» поверх реально идущей продажи.
...
async def bot_operation_guard(bot_id: int):
    """Гарантирует, что по одному боту одновременно идёт не более одной
    мутирующей операции (start/stop/delete). Если операция по этому боту уже
    идёт — бросает BotOperationInProgress ещё до входа в блок (повторный
    запрос ничего не запускает). Применяется на уровне HTTP-роутов, поэтому
    фоновый автозапуск из очереди ожидания (_do_start_bot напрямую) под эту
    защиту НЕ попадает.
    """
    async with _bot_ops_lock:
        if bot_id in _bot_ops_in_progress:
            raise BotOperationInProgress(
                "По этому боту уже выполняется операция. Дождитесь её завершения."
            )
...
class GridBotService:
    ...
    async def sync_commissions(bot_id: int) -> dict:
        ...
        async with db.acquire() as conn:
            trades = await conn.fetch(
                "SELECT id, order_id, direction, lots, lot_size, executed_at, commission "
                "FROM trade_history WHERE bot_id=$1 ORDER BY executed_at ASC",
                bot_id,
            )
        if not trades:
        ...
        for op in operations:
            ...
            if "BROKER_FEE" in op_type:
                ...
                deals.append({
                    "dt": op_dt,
                    "direction": "buy" if "BUY" in op_type else "sell",
        ...
        for t in trades:
            ...
            if abs(float(t["commission"] or 0.0) - fee_amount) < 1e-9:
                continue
            async with db.acquire() as conn:
                await conn.execute(
                    "UPDATE trade_history SET commission=$1 WHERE id=$2",
                    fee_amount, t["id"],
                )
            updated += 1
    ...
    async def _get_user_token(user_id: str) -> str:
        """Возвращает токен Т-Инвестиций владельца контейнера.

        В отличие от многопользовательской версии, токен хранится ТОЛЬКО в
        переменной окружения (задаётся при создании приложения на хостинге)
        и никогда не пишется в базу данных или логи.
        """
        token = settings.tinkoff_token
        if not token:
            logger.warning("Токен Т-Инвестиций не задан в переменных окружения")
            raise ValueError(
                "Токен Т-Инвестиций не задан. Укажите переменную окружения TINKOFF_TOKEN."
            )
        return token
    ...
    def _instance_from_bot_data(bot_data: dict, token: str) -> GridBotInstance:
        ...
        return GridBotInstance(
            ...
            class_code=bot_data["class_code"],
            P_low=float(bot_data["p_low"]),
            P_high=float(bot_data["p_high"]),
            capital=float(bot_data["capital"]),
            N=int(bot_data["n"]),
            initial_lots=int(bot_data.get("initial_lots") or 0),
            cash_remaining=float(bot_data.get("cash_remaining") or 0.0),
            current_price=float(bot_data["current_price"]) if bot_data.get("current_price") is not None else None,
            figi=figi,
        )
    ...
    async def estimate_max_levels(data: GridBotLevelsEstimateRequest) -> dict:
        """
        Рассчитывает, сколько уровней сетки (N) реально можно выставить при
        заданных капитале, диапазоне цен и текущей рыночной цене инструмента,
        чтобы каждый уровень (и на покупку, и на продажу) гарантированно
        получил хотя бы 1 лот. Используется и фронтендом (для подсказки
        пользователю), и бэкендом (для серверной валидации при создании бота).
        """
        ...
        # "Пробный" инстанс только для переиспользования методов запроса к
        # брокеру (получение инструмента и последней цены) — сделок не совершает.
        probe = GridBotInstance(
            bot_id=0,
        ...
        async with httpx.AsyncClient(verify=CA_BUNDLE_PATH, timeout=30.0) as client:
            ...
            step = int(min_step["units"]) + int(min_step["nano"]) / 1e9
        ...
        if not (data.P_low <= price <= data.P_high):
            raise ValueError(
                f"Текущая цена ({price:.2f} руб.) вне заданного диапазона "
                f"[{data.P_low:.2f}; {data.P_high:.2f}]"
            )
        ...
        if initial_lots < 1:
            return {
                ...
                "step": step,
                "initial_lots": 0,
                "cash_remaining_estimate": data.capital,
                "max_levels": 0,
                "reason": (
                    f"Капитала недостаточно для покупки даже одного лота по текущей цене. "
                    f"Требуется минимум {min_cost:.2f} руб. на покупку одного лота "
                    f"(при делении капитала пополам доступно {amount_to_spend:.2f} руб.)."
                ),
            }
        ...
        # Перебираем количество уровней N и находим максимальное значение,
        # при котором и уровни ниже цены (buy_count), и уровни выше цены
        # (sell_count) не превышают допустимых максимумов. Обе величины
        # монотонно не убывают при росте N, поэтому простой перебор корректен.
        max_levels = 0
        ...
        return {
            ...
            "cash_remaining_estimate": cash_remaining_est,
    ...
    async def create_bot(data: GridBotCreate) -> int:
        ...
        # Серверная проверка на случай, если фронтенд был обойдён (прямой
        # вызов API) — не даём создать бота с количеством уровней, при
        # котором часть уровней заведомо не получит ни одного лота.
        estimate = await GridBotService.estimate_max_levels(
            GridBotLevelsEstimateRequest(
                user_id=data.user_id,
                ticker=data.ticker,
                class_code=data.class_code,
                P_low=data.P_low,
        ...
        if data.N > estimate["max_levels"]:
            raise ValueError(
                f"Слишком много уровней ({data.N}) для указанного капитала. "
                f"При текущей цене ({estimate['current_price']:.2f} руб.) максимально "
                f"допустимое количество уровней: {estimate['max_levels']}. "
                f"Уменьшите количество уровней или увеличьте капитал."
            )
        ...
        async with db.acquire() as conn:
            row = await conn.fetchrow(
                ...
                data.user_id, data.ticker, data.class_code,
                data.P_low, data.P_high, data.capital, data.N
            )
            return row["id"]
    ...
    async def get_bot(bot_id: int) -> Optional[dict]:
        async with db.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT * FROM grid_bots WHERE id=$1 AND (type IS NULL OR type='simple')",
                bot_id
            )
    ...
    async def get_bots_by_user(user_id: str) -> List[dict]:
        async with db.acquire() as conn:
            rows = await conn.fetch(
                "SELECT * FROM grid_bots WHERE user_id = $1 AND (type IS NULL OR type='simple') AND status != 'archived' ORDER BY id DESC",
                user_id
            )
    ...
    async def get_active_orders(bot_id: int) -> List[dict]:
        async with db.acquire() as conn:
            rows = await conn.fetch(
                "SELECT * FROM grid_orders WHERE bot_id=$1 AND status='active'",
                bot_id
            )
            return [dict(r) for r in rows]
    ...
    async def _do_start_bot(bot_id: int) -> str:
        """Фактический запуск бота (инициализация/возобновление + мониторинг).

        Возвращает:
          - "started"  — бот успешно запущен;
          - "waiting"  — биржа закрыта, бот поставлен в очередь ожидания.
        Бросает ValueError при настоящей ошибке (не связанной с закрытой биржей).
        """
        bot_data = await GridBotService.get_bot(bot_id)
        if not bot_data:
            raise ValueError("Бот не найден")
        ...
        async with httpx.AsyncClient(verify=CA_BUNDLE_PATH, timeout=30.0) as client:
            try:
                ...
                # Биржа закрыта — ставим бот в очередь ожидания, а не в ошибку.
        ...
        # Убираем из очереди ожидания, если бот там был.
        await GridBotService._dequeue_waiting(bot_id)
        return "started"

    @staticmethod
    async def start_bot(bot_id: int) -> dict:
        """Публичный запуск бота из API.

        Возвращает словарь со статусом, который роутер отдаёт фронтенду:
          {"status": "started"} — бот запущен;
          {"status": "waiting", "opens_at": "<iso>"} — биржа закрыта, бот
          поставлен в очередь и стартует автоматически после открытия.

        Важно: при закрытой бирже метод НЕ бросает исключение (на фронте
        не должно быть ошибки), а возвращает status="waiting".
        """
        ...
        if result == "waiting":
            ...
            return {
                "status": "waiting",
                "opens_at": opens_at,
                "message": "Биржа закрыта. Бот поставлен в очередь и запустится автоматически после открытия торгов.",
            }
        return {"status": "started", "message": "Бот запущен"}

    @staticmethod
    async def _enqueue_waiting(bot_id: int, bot_data: dict) -> None:
        """Ставит бота в очередь ожидания открытия биржи и помечает 'waiting'."""
        ...
        try:
            ...
            instance = GridBotService._instance_from_bot_data(bot_data, token)
            async with httpx.AsyncClient(verify=CA_BUNDLE_PATH, timeout=30.0) as client:
                # Нужно узнать площадку инструмента, чтобы спрашивать расписание.
                try:
                    instr = await instance._get_instrument_by_ticker(client)
        ...
        async with db.acquire() as conn:
            await conn.execute(
                "UPDATE grid_bots SET status='waiting', updated_at=NOW() WHERE id=$1", bot_id
            )
        # Гарантируем, что планировщик запущен.
    ...
    def ensure_waiting_scheduler() -> None:
        """Запускает фоновый планировщик очереди ожидания, если он не запущен."""
        global _waiting_scheduler_task
        if _waiting_scheduler_task is None or _waiting_scheduler_task.done():
            _waiting_scheduler_task = asyncio.create_task(
                GridBotService._waiting_scheduler_loop()
            )
    ...
    async def _waiting_scheduler_loop(poll_seconds: int = 30) -> None:
        """Периодически проверяет очередь ожидания и запускает ботов,
        для которых биржа уже открылась. Работает, пока в очереди есть боты."""
        while True:
            try:
                async with _waiting_lock:
                    pending_ids = list(_waiting_bots.keys())
                if not pending_ids:
                    # Очередь пуста — планировщик завершает работу до следующей
                    # постановки в очередь (её вызов снова поднимет задачу).
    ...
    def get_waiting_bots() -> Dict[int, dict]:
        """Снимок очереди ожидания (для API/диагностики)."""
    ...
    async def stop_bot(bot_id: int) -> bool:
        ...
        # Если бот только ждал открытия биржи — просто убираем из очереди.
        await GridBotService._dequeue_waiting(bot_id)
        if bot_data["status"] == "waiting":
            async with db.acquire() as conn:
                await conn.execute(
                    "UPDATE grid_bots SET status='stopped', updated_at=NOW() WHERE id=$1", bot_id
                )
            return True
    ...
    async def delete_bot(bot_id: int, sell_position: bool = False) -> dict:
        """Удаляет (архивирует) бота.

        Порядок действий (независимо от sell_position):
          1. Останавливаем мониторинг.
          2. Отменяем ВСЕ активные стоп-заявки бота (тейк-профит на покупку и
             продажу в сетке) через CancelStopOrder.
          3. Если sell_position=True и у бота есть купленный актив — продаём
             его по рыночной цене (ORDER_TYPE_MARKET).
          4. Помечаем бота архивированным.

        Возвращает словарь с итогом: отменённые заявки, продан ли актив и т.п.
        """
        # Снимаем из очереди ожидания, если бот там.
        await GridBotService._dequeue_waiting(bot_id)
        ...
        # 1) Останавливаем цикл мониторинга (не даём боту действовать во время
        #    отмены/продажи).
        try:
        ...
        async with httpx.AsyncClient(verify=CA_BUNDLE_PATH, timeout=30.0) as client:
            ...
            # Убедимся, что знаем figi/lot_size/exchange для возможной продажи.
            try:
                instr = await instance._get_instrument_by_ticker(client)
                instance.figi = instr["figi"]
            ...
            # 2) Отменяем ВСЕ активные стоп-заявки бота.
            canceled_orders = await GridBotService._cancel_all_stop_orders(client, instance)

            # 3) Продажа купленного актива по рынку (по желанию пользователя).
            if sell_position:
                try:
        ...
        # Убираем из реестра активных ботов.
        if bot_id in _active_bots:
            del _active_bots[bot_id]
        ...
        async with db.acquire() as conn:
            await conn.execute(
                "UPDATE grid_bots SET status='archived', updated_at=NOW() WHERE id=$1", bot_id
            )

        logger.info(
            f"бот {bot_id} удалён: отменено заявок={canceled_orders}, "
            f"продано лотов={sold_lots}, продажа_запрошена={sell_position}"
        )
        return {
            "status": "archived",
    ...
    async def _cancel_all_stop_orders(client: httpx.AsyncClient, instance: "GridBotInstance") -> int:
        """Отменяет все активные стоп-заявки бота.

        Берёт заявки из БД (grid_orders со статусом 'active') и, для надёжности,
        сверяется со списком реальных активных стоп-заявок у брокера
        (GetStopOrders), чтобы не пропустить заявки, не попавшие в локальный учёт.
        Возвращает количество успешно отменённых заявок.
        """
        canceled = 0
        # Собираем идентификаторы стоп-заявок из локального учёта.
        order_ids: set = set()
        try:
            async with db.acquire() as conn:
                rows = await conn.fetch(
        ...
        # Дополняем реальным списком у брокера по инструменту бота.
        try:
            ...
            for so in broker_orders:
                ...
                if not sid:
                    continue
                same_instr = (
                    so.get("figi") == instance.figi
                    or (instance.instrument_uid and so.get("instrumentUid") == instance.instrument_uid)
                )
                if same_instr:
                    order_ids.add(sid)
        except Exception as e:
        ...
        for sid in order_ids:
            ...
            try:
                async with db.acquire() as conn:
                    await conn.execute(
                        "UPDATE grid_orders SET status='cancelled', updated_at=NOW() "
                        "WHERE order_id=$1 AND bot_id=$2",
                        sid, instance.bot_id,
                    )
            except Exception as e:
    ...
    async def _sell_all_position_market(client: httpx.AsyncClient, instance: "GridBotInstance") -> int:
        """Продаёт всю текущую позицию бота по инструменту рыночной заявкой.

        Количество лотов берём из реального портфеля брокера (GetPositions),
        а не из внутреннего учёта — так надёжнее. Возвращает число проданных
        лотов (0, если продавать нечего).
        """
        if not instance.account_id:
            instance.account_id = await instance._get_open_account_id(client)
        ...
        try:
            ...
            for sec in positions.get("securities", []) or []:
                if sec.get("figi") == instance.figi or (
                    instance.instrument_uid and sec.get("instrumentUid") == instance.instrument_uid
                ):
            ...
            # запасной вариант — внутренний учёт
        ...
        try:
            resp = await instance._post_order_async(
                client=client,
                instrument_id=instance.figi,
                quantity=held_lots,
        ...
        # Ждём исполнения и сохраняем сделку (для корректной статистики).
        for _ in range(90):
            ...
            if status == "EXECUTION_REPORT_STATUS_FILL" and executed >= held_lots:
                ...
                logger.info(
                    f"бот {instance.bot_id}: продано по рынку {held_lots} лотов "
                    f"по цене {price_per_share:.2f}"
                )
                break
        ...
        try:
            async with db.acquire() as conn:
                await conn.execute(
                    "UPDATE grid_bots SET initial_lots=$1 WHERE id=$2",
                    instance.initial_lots, instance.bot_id,
                )
        except Exception:
            pass
        return held_lots
    ...
    async def get_position_summary(bot_id: int) -> dict:
        """Возвращает сводку о текущей позиции бота для фронтенда: сколько
        лотов/акций реально в наличии (нужно, чтобы решить, предлагать ли
        продажу при удалении). Не совершает сделок."""
        bot_data = await GridBotService.get_bot(bot_id)
        if not bot_data:
            raise ValueError("Бот не найден")
        token = await GridBotService._get_user_token(bot_data["user_id"])
        ...
        return {
            "botId": bot_id,
            "hasPosition": held_lots > 0,
            "lots": held_lots,
            "lotSize": lot_size,
            "shares": held_lots * lot_size,
            "ticker": bot_data["ticker"],
        }

    @staticmethod
    async def get_grid_chart(bot_id: int, hours: int = 168) -> dict:
        """Данные для графика статистики бота: реальные свечи инструмента и
        уровни сетки (с ценами и текущим состоянием заявок).

        Возвращает:
            {
              "ticker": str,
              "lot_size": int,
              "step": float,              # минимальный шаг цены инструмента
              "grid_step": float,         # шаг между уровнями сетки, руб.
              "P_low": float, "P_high": float,
              "current_price": float | None,
              "candles": [{time, open, high, low, close, volume}, ...],
              "levels": [{index, price, side, lots, status}, ...],
            }
        side: 'buy' | 'sell' — на покупку/продажу относительно текущей цены.
        """
        bot_data = await GridBotService.get_bot(bot_id)
        if not bot_data:
            raise ValueError("Бот не найден")
        ...
        # Свечи и текущая цена требуют токен Т-Инвестиций и обращение к брокеру.
        # Оба шага НЕ критичны для графика: если токена нет или брокер недоступен,
        # мы всё равно строим и возвращаем сетку ордеров (по данным из БД) с
        # пустым массивом свечей, а не роняем весь эндпоинт ошибкой 404/500.
        # Иначе фронтенд получает не-JSON/ошибку и показывает «Сеть недоступна».
        try:
            token = await GridBotService._get_user_token(bot_data["user_id"])
            instance = GridBotService._instance_from_bot_data(bot_data, token)
            ...
            async with httpx.AsyncClient(verify=CA_BUNDLE_PATH, timeout=30.0) as client:
                ...
                # Для длинных диапазонов берём часовые свечи.
                candles = await instance._get_candles(
                    client, frm, now, interval="CANDLE_INTERVAL_HOUR"
                )
                current_price = await instance._get_last_price(client)
        except ValueError as e:
            # Нет токена Т-Инвестиций и т.п. — не критично, показываем только сетку.
        ...
        # Строим уровни сетки так же, как при инициализации бота.
        grid_step = (p_high - p_low) / n if n > 0 else 0.0
        levels: List[dict] = []
        if n > 0 and grid_step > 0:
            raw_levels = [p_low + i * grid_step for i in range(n + 1)]
            # Определяем текущий уровень (индекс), чтобы разметить buy/sell.
            i_cur = 0
            ...
            for i, price in enumerate(raw_levels):
                ...
                levels.append({
                    "index": i,
                    "price": round(price / step) * step if step else price,
                    "side": side,
                })
    ...
    async def restore_active_bots():
        # Поднимаем планировщик очереди ожидания на старте приложения.
        GridBotService.ensure_waiting_scheduler()

        async with db.acquire() as conn:
            rows = await conn.fetch(
                "SELECT * FROM grid_bots WHERE status='active' AND (type IS NULL OR type='simple')")
        for row in rows:
            bot_data = dict(row)
            try:
                token = await GridBotService._get_user_token(bot_data["user_id"])
        ...
        # Боты, оставшиеся в статусе 'waiting' после рестарта, возвращаем в очередь.
        async with db.acquire() as conn:
            waiting_rows = await conn.fetch(
                "SELECT * FROM grid_bots WHERE status='waiting' AND (type IS NULL OR type='simple')")
    ...
    async def shutdown_monitoring() -> None:
        """Аккуратное завершение при остановке контейнера.

        Останавливает фоновые задачи мониторинга, НЕ отменяя выставленные
        стоп-заявки и не меняя статус ботов в БД: при следующем старте
        контейнера боты со статусом 'active' будут восстановлены, а заявки
        у брокера продолжают работать и пока контейнер выключен.
        """
        global _waiting_scheduler_task
    ...
    async def collect_license_stats() -> dict:
        """Собирает компактную статистику по всем ботам для heartbeat лицензии.

        Эти данные сервер бота САМ отправляет платформе deflow раз в 15 минут
        (см. app/license.py). Платформа их только читает — доступа к базе бота
        и токену Т-Инвестиций у неё нет. Никогда не бросает исключение.
        """
        ...
        try:
            async with db.acquire() as conn:
                rows = await conn.fetch(
                    "SELECT * FROM grid_bots WHERE status != 'archived' "
                    "AND (type IS NULL OR type='simple') ORDER BY id"
                )
        except Exception as e:
        ...
        for row in rows:
            ...
            bots_out.append({
                ...
                "trading_paused": bool(inst.trading_paused) if inst else None,
                "license_hold": bool(inst._license_hold) if inst else None,
                "stats": stats,
            })

        return {
            "app": "t-invest-grid-bot",
            "bots_total": len(bots_out),
            "bots_active": sum(1 for b in bots_out if b["status"] == "active"),
            "license": license_manager.snapshot(),
            "bots": bots_out,
        }
