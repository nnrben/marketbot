...
class LicenseManager:
    ...
    def _load_cached(self) -> None:
        try:
            with open(self._lease_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            payload = self._verify_lease(data['lease'], data['sig'], expect_nonce=data.get('nonce'))
            if payload is not None and time.time() < payload['exp'] - _CLOCK_SKEW:
                self._apply_payload(payload, data.get('reason'))
                logger.info('Лицензия: восстановлена из кэша (статус=%s, действует ещё %d мин)', self._status, max(0, int((self._exp - time.time()) / 60)))
    ...
    def _save_cached(self, data: dict, nonce: str) -> None:
        try:
            payload = {'lease': data['lease'], 'sig': data['sig'], 'reason': data.get('reason'), 'nonce': nonce}
    ...
    def _verify_lease(self, lease_b64: str, sig_b64: str, expect_nonce: Optional[str]) -> Optional[dict]:
        try:
    ...
    def configured(self) -> bool:
        return bool(settings.license_id and settings.license_secret and settings.deflow_api_url)
    ...
    def snapshot(self) -> dict:
        return {'enforced': self.enforced, 'configured': self.configured, 'status': self._status, 'active': self.is_active(), 'tariff': self._tariff, 'reason': self.reason, 'expires_at': int(self._exp) if self._exp else None, 'expires_in_sec': max(0, int(self._exp - time.time())) if self._exp else None}

    async def fetch(self, stats: Optional[dict]=None) -> None:
        if not self.configured:
            if self.enforced and (not self._warned_unconfigured):
                self._warned_unconfigured = True
                logger.warning('Лицензия НЕ настроена (нужны LICENSE_ID, LICENSE_SECRET, DEFLOW_API_URL), а LICENSE_ENFORCE=true — бот будет на паузе. Получите лицензию в личном кабинете deflow и задайте переменные окружения. Для локального теста без платформы: LICENSE_ENFORCE=false.')
            return
        from app.services.grid_bot.config import CA_BUNDLE_PATH
        nonce = secrets.token_urlsafe(16)
        url = settings.deflow_api_url.rstrip('/') + '/api/bot/telemetry'
        body = {'license_id': settings.license_id, 'nonce': nonce, 'ts': int(time.time()), 'fingerprint': self._fingerprint, 'stats': stats or {}}
        try:
            async with httpx.AsyncClient(verify=CA_BUNDLE_PATH, timeout=15.0) as client:
                resp = await client.post(url, json=body, headers={'Authorization': f'Bearer {settings.license_secret}'})
        except Exception as e:
            left = max(0, int((self._exp - time.time()) / 60)) if self._exp else 0
            logger.warning('Лицензия: платформа недоступна (%s). Бот доживает на текущем lease ещё ~%d мин.', e, left)
            return
        if resp.status_code in (401, 403):
            logger.error('Лицензия: платформа отклонила heartbeat (%s). Проверьте LICENSE_ID/LICENSE_SECRET. Бот встаёт на паузу.', resp.status_code)
        ...
        if resp.status_code != 200:
            logger.warning('Лицензия: неожиданный ответ платформы %s — сохраняю текущий lease.', resp.status_code)
            return
        try:
            data = resp.json()
        except Exception:
        ...
        if prev_status != self._status:
            logger.info('Лицензия: статус %s -> %s (тариф=%s, причина=%s, действует %d мин)', prev_status, self._status, self._tariff, self.reason, max(0, int((self._exp - time.time()) / 60)))
license_manager = LicenseManager()

async def license_loop(stats_provider: Callable[[], Awaitable[dict]], on_change: Optional[Callable[[bool], Awaitable[None]]]=None) -> None:
