...
class Settings(BaseSettings):
    ...
    tinkoff_token: str = Field('', validation_alias=AliasChoices('TINKOFF_TOKEN', 'T_INVEST_TOKEN', 'TBANK_TOKEN'))
    ...
    allow_existing_position: bool = Field(False, validation_alias=AliasChoices('ALLOW_EXISTING_POSITION'))
    reset_orders_on_start: bool = Field(False, validation_alias=AliasChoices('RESET_ORDERS_ON_START'))
    ...
    license_poll_seconds: int = Field(900, validation_alias=AliasChoices('LICENSE_POLL_SECONDS'))
    license_enforce: bool = Field(True, validation_alias=AliasChoices('LICENSE_ENFORCE'))
    license_public_key: str = Field('', validation_alias=AliasChoices('LICENSE_PUBLIC_KEY'))
    data_dir: str = Field('/app/data', validation_alias=AliasChoices('DATA_DIR'))
    ca_bundle_path: str = Field('/app/certs/ca-bundle.crt', validation_alias=AliasChoices('CA_BUNDLE_PATH'))
    ...
    def validate_runtime(self) -> None:
        if not self.tinkoff_token:
            logger.warning('Не задан TINKOFF_TOKEN. Контейнер запустится, но торговые операции будут недоступны, пока токен не задан. Добавьте переменную окружения TINKOFF_TOKEN в настройках приложения (Timeweb Cloud Apps -> Переменные окружения).')
        if self.bot_configured:
            errors = []
            if self.p_high <= self.p_low or self.p_low <= 0:
                errors.append('P_LOW/P_HIGH: нижняя граница должна быть > 0 и меньше верхней')
            if self.capital <= 0:
            ...
            if errors:
                logger.warning('Некорректные параметры бота в переменных окружения (бот не будет создан автоматически, задайте параметры через API): %s', '; '.join(errors))
        else:
            logger.info('Переменная TICKER не задана — бот не создаётся из окружения. Параметры бота задаются удалённо через HTTP API (заголовок X-API-Key, переменная API_AUTH_TOKEN).')
settings = Settings()
